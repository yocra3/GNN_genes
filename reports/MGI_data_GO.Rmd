---
title: "Overview of mice KO data present in GO"
author: "Carlos Ruiz"
date: "`r Sys.Date()`"
...
```{r}
all_genos <- unlist(strsplit(genotype_phenotype_go$MGIMarkerGenotype, ","))
all_genes <- unlist(strsplit(all_genos, "/"))
all_genes <- all_genes[grep("MGI", all_genes)]
```

We selected GO terms with more than 5 genes, without children and containing exclusive KO genes (i.e. a KO gene was only present in that GO term). Overall, `r length(unique(sing_leaves_go_gene$GO_ID))` GO terms were selected. After selecting those combinations of KO alleles were all genes were present in at least a GO terms, we had `r nrow(genotype_phenotype_go)` combinations of gene alleles and genetic backgrounds corresponding to `r length(unique(all_genes))` different genes. `r sum(genotype_phenotype_go$PhenotypeID == "")` combinations of genes corresponded to alleles without phenotypic effect.

```{r}
no_effect <- subset(genotype_phenotype_go, TopPhenotypeIDs == "")$MGIMarkerGenotype 
...
```{r}
marker_list <- strsplit(subset(genotype_phenotype_go, !is.na(PhenotypeID))$MGIMarkerGenotype, ",")
...
```{r}
all_phenos <- strsplit(genotype_phenotype_go$TopPhenotypeIDs, ",")
