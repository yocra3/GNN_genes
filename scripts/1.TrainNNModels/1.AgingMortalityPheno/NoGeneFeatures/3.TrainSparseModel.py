...
from NNsparsemodel import sparseNN
...
gene_names = pd.read_csv("results/NNmodels/data/gene_order.tsv", sep = "\t", header = None)
gene_map =  pd.read_csv("results/NNmodels/data/gene_go_map.tsv", sep = "\t")
n_gos = len(gene_map["GO_ID"].unique())

genes = gene_names.iloc[:, 0].values
gene_map_input = gene_map[gene_map['MGIMarkerID'].isin(genes)]

## Create go-genes dictionary
gos = gene_map_input['GO_ID'].unique()
go_dict, gen_dict = dict(zip(gos, range(len(gos)))), dict(zip(genes, range(len(genes))))
rel_dict = defaultdict(list)
for go, gen in zip(gene_map_input['GO_ID'], gene_map_input['MGIMarkerID']):
    rel_dict[gen_dict[gen]].append(go_dict[go])

## Define model
model0 = sparseNN(n_genes, len(gos), rel_dict, 10, 1, device = device)
